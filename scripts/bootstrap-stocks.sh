...
# Does not start Reth, the node, or the app. Run start-stocks-07.sh after this.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LABS="$(cd "$SCRIPT_DIR/../.." && pwd)"
...
if [[ ! -d "$LABS/tokenized-stocks-app/frontend/node_modules" ]]; then
  echo "npm install (frontend)"
  (cd "$LABS/tokenized-stocks-app/frontend" && npm install)
else
